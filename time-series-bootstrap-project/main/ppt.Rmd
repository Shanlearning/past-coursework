...
# Introduction to Bootstrap
 ...
 1.The normal bootstrap methods: 
...
```{r parameters}
# specify parameters:
phi <- c(.8); n <- 300; sigma <- 2
X <- arima.sim(n=n,list(ar=phi,sd=sigma))
fit<-arima(X,order=c(1,0,0))
fit$coef[1]

# how do generate sample
...
# Block Boostrap: resample blocks, do not require residual to be i.i.d.
...
Stationary bootstrap: Block size is a random variable
...
Here is how we generate a SB sample
```{r SBcode2}
# have a look to determine appropriate block size
expectedl<-25 ; gprob=1/expectedl
X.star<-numeric(n)
...
```{r interestrate1,echo = FALSE}
plot(dat)
```

#plot
```{r interestrate2,echo = FALSE}
acf(diff(dat$GS10))
```

#plot
```{r interestrate3,echo = FALSE}
plot(diff(dat$GS10),type="l",main="plot of the 1st difference")
```

#Fit with a arima(1,1,0) model
```{r interestrate4,echo=FALSE}
...
```{r interestresidualbase , echo = FALSE}
X <- diff(dat$GS10)
fit<-arima(X,order=c(1,0,0))
