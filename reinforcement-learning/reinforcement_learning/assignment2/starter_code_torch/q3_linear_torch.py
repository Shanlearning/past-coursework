import os
os.chdir("C:\\Users\\zhong\\Dropbox\\statistics\\CS 790\\assignment2\\starter_code_torch")
...
class Linear(DQN):
    ...
    def get_q_values(self, state, network='q_network'):
        ...
        ################ YOUR CODE HERE - 3-5 lines ##################
         # batch_size = 32, data 5*5 one hot
    ...
    def update_target(self):
        ...
        for target_param, pred_param in zip(self.target_network.parameters(), self.q_network.parameters()):
    ...
    def calc_loss(self, q_values : Tensor, target_q_values : Tensor,
        ...
        actions = F.one_hot(actions.to(torch.int64), num_classes=num_actions)
        loss = F.mse_loss(q_samp,torch.max(q_values * actions, dim=1)[0])
        return loss
        ##############################################################
        ######################## END YOUR CODE #######################
