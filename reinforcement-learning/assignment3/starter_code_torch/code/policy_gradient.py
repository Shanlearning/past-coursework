...
class PolicyGradient(object):
    ...
    def init_policy(self):
        ...
        self.network = build_mlp( input_size = self.observation_dim, output_size = self.action_dim, n_layers = self.config.n_layers, size = self.config.layer_size)
    ...
    def get_returns(self, paths):
        ...
        for path in paths:
            ...
            returns = []
            for t in range(T):
                k = np.linspace(start = 0, stop = T-t-1, num=T-t, endpoint = True)
                G_t = sum(np.power(gamma,k) *  rewards[t:T])
                returns.append(G_t)    
            #######################################################
            #########          END YOUR CODE.          ############
    ...
    def normalize_advantage(self, advantages):
        ...
        normalized_advantages = (advantages - np.mean(advantages,axis=0)) / np.std(advantages,axis=0)
        #######################################################
        #########          END YOUR CODE.          ############
    ...
    def update_policy(self, observations, actions, advantages):
        ...
        loss = -torch.mean( distribution.log_prob(actions) * advantages )
